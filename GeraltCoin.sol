...
contract WITCHER is ERC20, Ownable, Shared {
   ...
   uint256 public WitcherPursueFee = 4;
   uint256 public BardsPayFee = 1;
   ...
   address public WitcherPursueAddress = 0xEeAA596d77EC5A80fB87aDB6D6F806b5aa9651CB;  //Marketing wallet
   address public BardsPayAddress = 0xd7F282045849BE537AAC53f9F512cD90bB2c93f1;    //Dev wallet
   ...
   constructor() ERC20("Geralt Coin", "WITCHER") {
       ...
       excludeFromFees(WitcherPursueAddress, true);
       excludeFromFees(BardsPayAddress, true);
   ...
   function setWitcherPursueFee(uint256 value) external onlyOwner {
       WitcherPursueFee = value;
   }
  
   function setBardsPayFee(uint256 value) external onlyOwner {
       BardsPayFee = value;
   }
   ...
   function setMaxTx(uint256 maxTxVal) external onlyOwner {
       require (maxTxVal > 0, "Can't set max Tx to 0");
       maxTxAmount = totalSupply() * maxTxVal / 1000;
   }
   
   function setMaxWallet(uint256 maxWalletVal) external onlyOwner {
       require (maxWalletVal > 0, "Max wallett can't be 0");
       maxWallet = maxWalletVal;
   }
   ...
   function _transfer(
       ...
       require (tradingOpened || isExcludedFromFees[from], "Trading paused");
       require(from != address(0), "ERC20: transfer from the zero address");
       require(to != address(0), "ERC20: transfer to the zero address");
       require(isBlacklistedUntil[from] < block.timestamp && isBlacklistedUntil[to] < block.timestamp, "Blacklisted address");
       ...
       if( !swapping &&
           ...
           require(amount <= maxTxAmount, "Max Tx Amount reached");
           require(balanceOf(to) + amount <= maxWallet, "Max Wallet reached");
       }
 
       uint256 contractTokenBalance = balanceOf(address(this));
       uint256 totalFees = WitcherPursueFee + BardsPayFee;
       ...
       if( canSwap &&
           ...
           uint256 witchercoin = contractTokenBalance * WitcherPursueFee / totalFees;
           swapAndSendToFee(witchercoin, WitcherPursueAddress);
           uint256 bardcoin = contractTokenBalance * BardsPayFee / totalFees;
           swapAndSendToFee(bardcoin, BardsPayAddress);
   ...
   function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
       ...
       if (ethAmount - ethFromLiquidity > 0)
           payable(WitcherPursueAddress).sendValue(ethAmount - ethFromLiquidity);
   }
